...
func (db DB) GetConfigs(exchangeName string) []Configs {
	var configs []Configs

	result := db.conn.Find(&configs, "exchange = ?", exchangeName)
...
func (db DB) GetConfig(symbol string, exchangeName string) Configs {
	var config Configs

	result := db.conn.First(&config, "symbol = ?", symbol, "exchange = ?", exchangeName)
...
func (db DB) UpdateConfigTradingEnabled(symbol string, exchangeName string, enabled bool) error {
	config := Configs{
		Symbol:   symbol,
...
func (db DB) UpdateConfigAllowedAmount(symbol string, exchangeName string, amount float64) error {
	config := Configs{
		Symbol:   symbol,
		Exchange: exchangeName,
	}
