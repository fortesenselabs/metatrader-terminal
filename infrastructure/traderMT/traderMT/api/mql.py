...
class RequestError(Exception):
    def __init__(self, last_error, description, function):
        self.last_error = last_error
        self.description = description
        self.function = function
        super().__init__(f"{description} (Error Code: {last_error}) in {function}")

class Functions:
    def __init__(self, host=None, debug=False):
        self.HOST = host or "localhost"
        self.SYS_PORT = 15557  # REP/REQ port
        self.DATA_PORT = 15556
        self.LIVE_PORT = 15557
        self.STR_PORT = 15558

        # initialise socket
        self.sys_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.sys_socket.settimeout(10)

        self.data_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.data_socket.settimeout(10)

        self.live_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.live_socket.settimeout(10)

        self.streaming_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.streaming_socket.settimeout(10)

        # connect to server sockets
        try:
            self.sys_socket.connect((self.HOST, self.SYS_PORT))
            self.data_socket.connect((self.HOST, self.DATA_PORT))
            self.live_socket.connect((self.HOST, self.LIVE_PORT))
            self.streaming_socket.connect((self.HOST, self.STR_PORT))
        except socket.error as e:
            raise ConnectionError(f"Binding ports ERROR: {e}")

    def _send_request(self, data: dict) -> None:
        """Send request to server via socket"""
        try:
            self.sys_socket.sendall((json.dumps(data)).encode())
        except socket.error as e:
            raise ConnectionError(f"Sending request ERROR: {e}")

    def _pull_reply(self) -> Union[dict, Any]:
        """Get reply from server via socket with timeout"""
        try:
            reply = self.data_socket.recv(4096).decode()
            try:
            ...
            raise ConnectionError(f"Data socket timeout ERROR: {e}")
    ...
    def Command(self, **kwargs) -> dict:
        ...
        # return server reply
        response = self._pull_reply()
        if "error" in response and response["error"]:
            raise RequestError(response["lastError"], response["description"], response["function"])
...
class Metatrader:
    ...
    def symbols(self):
        """
            TODO: `LISTSYMBOLS` api command not working
        """
        df = None
        try:
