...
class MetaTraderDataProcessor:
    ...
    def get_historical_klines(
        self,
        symbol: str = "EURUSD",
        time_frame: Optional[TimeFrame] = TimeFrame.D1,
        ...
        if start <= 0 or start is None:
            start = (
                datetime.now(timezone.utc) - (time_frame.to_timedelta() * limit)
            ).timestamp()

        if end <= 0 or end is None:
            end = datetime.now(timezone.utc).timestamp()

        self.dwx.get_historic_data(symbol, time_frame.value, start, end)
