...
class MTSocketClient:
    ...
    async def connect(self):
        ...
        while retries < self.max_retries:
            try:
                ...
                retries += 1
                await asyncio.sleep(self.max_retry_command_seconds)
    ...
    async def receive_chunked_data(self, reader: asyncio.StreamReader):
        ...
        while retries < max_retries:
            try:
                ...
                if (
                    ...
                    self.logger.error(
                        f"Socket receive error: {e}, retrying in {retry_delay} seconds..."
                    )
    ...
    async def send_command(self, command: str, content: str) -> Optional[Dict]:
        if self.verbose:
        ...
        try:
            ...
            self.logger.exception(f"Failed to send command: {e}")
            self.connected = False

        try:
            response = await self.receive_chunked_data(self.reader)
        except RuntimeError:
            self.logger.exception(f"Socket receive error: {e}")

        self.logger.info(f"Received response(send_command): {response}")
        self.reader_response = response
        return self.reader_response
    ...
    async def get_account_info(self) -> Dict:
        ...
        Get Account Info
        """
        await self.send_command("ACCOUNT_INFO", "")
        print("self.reader_response: ", self.reader_response)
        if await self.wait_for_event(lambda: len(self.reader_response) > 0):
            data = self.reader_response
            print("data: ", data)
            if data is None:
                return None
    ...
    async def get_active_symbols(self, symbol="") -> Dict:
        """
        ...
        """
        data = await self.send_command("GET_ACTIVE_SYMBOLS", str(symbol))
        # if await self.wait_for_event(lambda: len(self.symbols_data) > 0, timeout=5):
        if data is None:
            return None

        if self.event_handler is not None:
            print("self.symbols_data: ", self.symbols_data)
            self.event_handler.on_symbols_data(self.symbols_data)
...
# if await self.pubsub.wait_for_event(
#     lambda: len(self.symbols_data) > 0, timeout=5
# ):
# return self.symbols_data
